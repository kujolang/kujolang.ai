---
title: "WebOps Cannibalization"
custom_url: "webops-cannibalization"
description: "Use when identifying likely intent competition while separating normal overlap."
excerpt: "Use when identifying likely intent competition while separating normal overlap."
featured_image: "/assets/images/ecosystem/skill-webops-cannibalization.webp"
social_image: "/assets/images/social/webops-cannibalization.jpg"
section: "Agent Skill"
order: "1130"
install_command: "git clone --branch v0.7.0 --depth 1 https://github.com/kujolang/kujo-skills.git && mkdir -p ~/.codex/skills && cp -R kujo-skills/skills/webops-cannibalization ~/.codex/skills/"
github_url: "https://github.com/kujolang/kujo-skills/blob/v0.7.0/skills/webops-cannibalization/SKILL.md"
launch_story: "One of 135 focused, repository-backed skills in the Kujo Skills 0.7.0 technical preview."
scope_note: "Guidance only. The agent host must enforce permissions, capability limits, and approval boundaries."
version: "0.7.0"
last_updated: "2026-09-08"
keywords: "WebOps Cannibalization, Kujo Agent Skill, webops-cannibalization, repository-backed agent guidance"
seo_title: "WebOps Cannibalization — Kujo Agent Skill"
seo_description: "Use when identifying likely intent competition while separating normal overlap."
---

## What it covers

Use when identifying likely intent competition while separating normal overlap.

Run the install command from a new working directory. If you already have the released checkout, copy the complete skill folder from it instead of cloning again.

## Released guidance

- intent comparison.
- candidate classification.
- query/page evidence.
- bounded recommendation.

## Release boundary

This page reflects the 0.7.0 technical preview on 2026-09-08. The skill provides repository-backed guidance; the agent host remains responsible for permissions and enforcement.

## Source

- [Read the complete webops-cannibalization skill on GitHub](https://github.com/kujolang/kujo-skills/blob/v0.7.0/skills/webops-cannibalization/SKILL.md)
- [Browse the released Kujo Skills Index](https://github.com/kujolang/kujo-skills/blob/v0.7.0/SKILLS_INDEX.md)
